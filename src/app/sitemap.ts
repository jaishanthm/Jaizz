...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [seo, flags, pageSeoList] = await Promise.all([
      ...
      getEnabledFeatureFlags().catch(() => []),
      prisma.pageSEO.findMany({ select: { path: true, noindex: true } }).catch(() => []),
    ]);

    const base = seo?.canonicalBaseUrl ?? defaultBase;
  ...
  if (eligibleKeys.has("projects")) {
    const projects = await prisma.project.findMany({ where: { visible: true }, select: { slug: true, updatedAt: true } }).catch(() => []);
    entries.push(...projects.map((p) => ({ url: `${base}/projects/${p.slug}`, lastModified: p.updatedAt })));
  }
  if (eligibleKeys.has("research")) {
    const research = await prisma.research.findMany({ where: { visible: true, disclosureStatus: "PUBLISHED" }, select: { slug: true, updatedAt: true } }).catch(() => []);
  ...
  return entries;
    ...
    return [
      { url: defaultBase, lastModified: new Date() },
      { url: `${defaultBase}/about` },
      { url: `${defaultBase}/projects` },
      { url: `${defaultBase}/skills` },
      { url: `${defaultBase}/experience` },
      { url: `${defaultBase}/contact` },
    ];
  }
}
